...
private:
    ...
    int writen(int sockfd, char const* buf, int n); // write n bytes to socket
...
/* known protocols */
/* ##extension */
static char const* URLC_kp[] = {"http://", "file:", "ftp://", NULL};
...
/* writes n bytes to socket.
   ...
   */
int
urlc::writen(int lsockfd, char const* buf, int n)
{
    int nwritten = 0;
    int nleft = n;
    while(0 < nleft) {
...
urlc::http_req(int sockfd)
{
    ...
    int tot_len = 0;
    char* p = NULL;
    char const* req_form[] = { "GET ", path, " HTTP/1.0\r\n",
	       		       "Host: ", host , "\r\n",
